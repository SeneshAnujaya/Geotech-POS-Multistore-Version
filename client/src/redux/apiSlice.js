...
const apiSlice = createApi({
  ...
  endpoints: (builder) => ({
    ...
    fetchPaginatedProducts: builder.query({
      query: ({ page = 0, limit = 20, searchTerm = "", storeId }) =>
        `/products/getpaginationProducts?page=${page}&limit=${limit}&searchTerm=${searchTerm}&storeId=${storeId}`,
      providesTags: ["Products"],
    }),
    ...
    fetchPaginatedSales: builder.query({
      query: ({ page = 0, limit = 50, searchTerm = "", storeId }) =>
        `/sales/getpaginationSales?page=${page}&limit=${limit}&searchTerm=${searchTerm}&storeId=${storeId}`,
      providesTags: ["Sales"],
    }),

    fetchDueSales: builder.query({
      query: ({ searchTerm = "", storeId }) =>
        `/sales/getDueSales?searchTerm=${searchTerm}&storeId=${storeId}`,
      porvidesTags: ["Sales"],
    }),

    fetchReturnCancelSales: builder.query({
      query: ({ searchTerm, storeId }) =>
        `/sales/getReturnCancel?searchTerm=${searchTerm}&storeId=${storeId}`,
      providesTags: ["Sales"],
    }),
    ...
    fetchWholesaleClients: builder.query({
      query: ({ searchTerm = "" }) =>
        `/wholesaleClient/getBulkBuyers?searchTerm=${searchTerm}`,
    ...
    fetchStocks: builder.query({
      query: ({ page = 0, limit = 20, searchTerm = "", storeId }) =>
        `/stocks?page=${page}&limit=${limit}&searchTerm=${searchTerm}&storeId=${storeId}`,
      providesTags: ["Stocks"],
    }),
    ...
    fetchRepairjobs: builder.query({
      query: ({ page = 0, limit = 20, searchTerm = "", storeId }) =>
        `/repair?page=${page}&limit=${limit}&search=${searchTerm}&storeId=${storeId}`,
      providesTags: ["Repair"],
    }),
    ...
    fetchSingleRepairJob: builder.query({
      query: ({ repairJobId, storeId }) =>
        `/repair/${repairJobId}?storeId=${storeId}`,
      providesTags: ["Repair"],
    }),
    updateRepairJob: builder.mutation({
      query: ({ id, selectedStoreId, updatedData }) => ({
    ...
    updateRepairDetails: builder.mutation({
      query: ({ repairJobId, storeId, updatedData }) => ({
        url: `/repair/${repairJobId}?storeId=${storeId}`,
        method: "PATCH",
        body: updatedData,
      }),
      invalidatesTags: ["Repair"],
